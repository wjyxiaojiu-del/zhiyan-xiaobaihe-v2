"""AI 问答服务 - DeepSeek API + 本地回退"""
from config import DEEPSEEK_API_KEY, DEEPSEEK_BASE_URL, DEEPSEEK_MODEL

# 延迟导入 openai，未配置 API key 时不影响启动
_client = None
...
def generate_answer(query, context="", history=None):
    """生成 AI 回答，优先使用 DeepSeek API，回退到本地规则引擎"""
    client = _get_client()

    if client:
        return _call_deepseek(client, query, context, history)
    else:
        return _local_fallback(query, context)


def _call_deepseek(client, query, context, history=None):
...
> 提示：配置 DEEPSEEK_API_KEY 环境变量后，将启用 DeepSeek AI 获得更智能的回答。"""
